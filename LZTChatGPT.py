...
token_lolz = ""
forum_id = 876 # Если не знаете где взять нужный для вас раздел, напишите в комментариях в теме. 876 - тестовый раздел. 585 - тематические вопросы
headers = {
...
def ThabAIGen(text_lolz:str) -> str:
    logger.info('Получаем ответ от ChatGPT')
    text = g4f.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": f"{settings_gpt} {text_lolz}"}]
    )
    logger.success('Ответ от ChatGPT получен')
    return text
